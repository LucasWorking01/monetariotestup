...
### 3. Relatório em PDF
Ainda na aba Relatório, escolha um intervalo de meses (por exemplo, de Janeiro
a Junho) e clique em **Gerar PDF**. O app monta um resumo do período — totais,
gastos por categoria, entradas por categoria e a evolução mês a mês — e abre a
janela de impressão do navegador. Basta escolher **"Salvar como PDF"** como
destino da impressão para baixar o documento. Não lista cada lançamento
individualmente: é um resumo, para um relatório rápido de conferir ou enviar.
